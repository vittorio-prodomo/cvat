...
class DummyModel:
    def handle(self, *, image, obj_bbox, mapping):
        ...
        assert mapping == {'mild_crack': {'name': 'mild_crack', 'attributes': {}}}
        return [{
            'label': 'mild_crack',
            'type': 'mask',
...
def test_handler_decodes_image_and_returns_shapes_json():
    ...
    event = SimpleNamespace(body={
        'image': encode_image(),
        'obj_bbox': [[1, 1], [3, 3]],
        'mapping': {'mild_crack': {'name': 'mild_crack', 'attributes': {}}},
    })

    response = main.handler(context, event)

    assert response.status_code == 200
    assert json.loads(response.body)['shapes'][0]['label'] == 'mild_crack'
