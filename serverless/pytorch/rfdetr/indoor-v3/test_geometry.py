...
@pytest.mark.parametrize("shape,resized,pad", [
    ((11, 17), (652, 1008), (178, 0)),
    ((17, 11), (1008, 652), (0, 178)),
    ((10, 17), (593, 1008), (207, 0)),
    ((7, 16), (441, 1008), (283, 0)),
    ((13, 16), (819, 1008), (94, 0)),
    ((1, 2000), (1, 1008), (503, 0)),
    ((2000, 1), (1008, 1), (0, 503)),
    ((1008, 1008), (1008, 1008), (0, 0)),
])
def test_letterbox_uses_validation_resize_black_padding_and_rounding(
    ...
    top, left = pad
    expected = np.zeros((1008, 1008, 3), dtype=np.uint8)
...
@pytest.mark.parametrize("shape", [(1007, 1008), (1008, 1007), (51, 83), (83, 51)])
def test_mask_restoration_removes_odd_padding_and_keeps_received_frame_edges(load_indoor, shape):
    ...
    mask = np.zeros((1008, 1008), dtype=bool)
...
def test_mask_restoration_preserves_topology_at_native_size(load_indoor):
    geometry = load_indoor("geometry")
    _, frame = geometry.letterbox(np.zeros((1007, 1008, 3), dtype=np.uint8))
    source = np.zeros((1007, 1008), dtype=bool)
...
@pytest.mark.parametrize("shape", [(504, 504), (1, 1008, 1008), (1008, 1007)])
...
def test_preprocessing_matches_real_albumentations_validation_transforms(load_indoor, monkeypatch):
    ...
    transform = albumentations.Compose([
        albumentations.LongestMaxSize(max_size=1008),
        albumentations.PadIfNeeded(min_height=1008, min_width=1008, border_mode=0, fill=0, fill_mask=0),
    ])
    rng = np.random.default_rng(42)
    for shape in ((11, 17), (17, 11), (1007, 1008), (1008, 1007), (1, 2000), (2000, 1), (701, 1033)):
