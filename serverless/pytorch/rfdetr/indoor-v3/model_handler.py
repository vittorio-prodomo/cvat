"""Pinned Indoor v3 inference and native CVAT detector annotations."""
...
def load_checkpoint_state_dict(checkpoint_path: Path, torch_module) -> dict:
    ...
    with Path(checkpoint_path).open("rb") as stream:
        ...
        if digest.hexdigest() != CHECKPOINT_SHA256:
            raise ValueError("Indoor checkpoint SHA256 does not match the pinned artifact")
        stream.seek(0)
    ...
    if not isinstance(checkpoint, Mapping) or not isinstance(checkpoint.get("state_dict"), Mapping):
        raise ValueError("Indoor checkpoint must contain a Lightning state_dict")
    state = {
    ...
    if not state:
        raise ValueError("Indoor checkpoint contains no model.* state_dict entries")
    return state
...
class IndoorBackend:
    ...
    def __init__(self, checkpoint_path=None, device="cuda", logger=None):
        ...
        if logger is not None:
            logger.info(f"Indoor v3 ready: checkpoint_sha256={CHECKPOINT_SHA256}")
            ...
            try:
                ...
                for key in ("pred_logits", "pred_masks"):
                    if not torch.isfinite(outputs[key]).all().item():
                        raise ValueError(f"Indoor returned nonfinite {key}")
                predictions = self._adapter.postprocess_for_inference(
                    outputs, image_sizes=(INPUT_SIZE, INPUT_SIZE)
                )
                if len(predictions) != 1:
                    raise ValueError("Expected one Indoor prediction batch")
                return {
...
class ModelHandler:
    def __init__(self, logger=None, backend=None):
        self.backend = backend if backend is not None else IndoorBackend(logger=logger)
    ...
    def handle(self, *, image, threshold=None) -> list[dict]:
        ...
        if scores.ndim != 1 or labels.shape != scores.shape:
            raise ValueError("Inconsistent Indoor prediction dimensions")
        if masks.shape != (len(scores), INPUT_SIZE, INPUT_SIZE):
            raise ValueError(f"Expected Indoor masks with shape (N, {INPUT_SIZE}, {INPUT_SIZE})")
        if masks.dtype != np.bool_:
            raise ValueError("Expected decoded boolean Indoor masks")
        if not np.issubdtype(labels.dtype, np.integer) or np.any((labels < 0) | (labels >= len(CLASS_NAMES))):
            raise ValueError("Indoor returned an invalid class ID")
        if not np.issubdtype(scores.dtype, np.number) or not np.all(np.isfinite(scores)) or np.any((scores < 0) | (scores > 1)):
            raise ValueError("Indoor returned an invalid confidence score")
        annotations = []
