...
MODULE_NAME = 'eagle_combined_v5_main'
...
class DummyModel:
    def handle(self, *, image, obj_bbox, mapping, confidence_threshold=None):
        ...
        assert mapping == {
            '(A13) danno_urto': {'name': 'bridge_damage', 'attributes': {}},
            '(C6) superf_bagn_dilav_percolaz': {'name': 'bridge_stain', 'attributes': {}},
        }
        return [
            {
                'label': 'bridge_damage',
            ...
            },
            {
                ...
                'type': 'mask',
                'points': [0, 4, 4, 4, 0, 0, 3, 3],
                'attributes': [{'spec_id': 0, 'value': '0.85'}],
            },
        ]
...
def test_handler_decodes_image_and_returns_shapes_json():
    ...
    shapes = json.loads(response.body)['shapes']
    assert len(shapes) == 2
    assert shapes[0]['label'] == 'bridge_damage'
    assert shapes[1]['label'] == 'bridge_stain'
...
def test_handler_logs_request_summary():
    ...
    assert any(
        message == (
            ...
            'bbox=[[1, 1], [3, 3]] '
            'mapping_keys=2 '
            "mapping_labels=['(A13) danno_urto', '(C6) superf_bagn_dilav_percolaz']"
        )
        for message in messages
    )
    assert any(
        message == 'RF-DETR combined response summary: returned_shapes=2'
        for message in messages
    )
