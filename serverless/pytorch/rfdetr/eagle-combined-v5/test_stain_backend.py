...
LOCAL_MODULE_NAMES = ('postprocess', 'stain_backend')
...
def mock_torch():
    ...
    ensuring torch mock is available when stain_backend imports it.
    """
    class MockTensor:
    ...
    # Force reimport of stain_backend so it picks up the mocked torch
    if 'stain_backend' in sys.modules:
        del sys.modules['stain_backend']
    ...
    # Clean up stain_backend to force reimport next time
    if 'stain_backend' in sys.modules:
        del sys.modules['stain_backend']


# Import stain_backend functions inside each test, not at module level
...
def test_parse_map_score_reads_epoch_style_checkpoint_names(mock_torch):
    from stain_backend import parse_map_score
...
def test_find_best_checkpoint_picks_highest_map(mock_torch, tmp_path):
    from stain_backend import find_best_checkpoint
...
def test_load_checkpoint_state_dict_strips_lightning_model_prefix(mock_torch, tmp_path):
    from stain_backend import load_checkpoint_state_dict
    import sys
...
def test_default_paths_match_manifest_opt_bdd_mount(mock_torch):
    ...
    expected_checkpoint_dir = Path("/opt/bdd/runs/echo-combined-v5/stain_round1/checkpoints")
    expected_config_path = Path("/opt/bdd/runs/echo-combined-v5/stain_round1/config.yaml")
    ...
    assert RFDETRStainBackend.DEFAULT_CHECKPOINT_DIR == expected_checkpoint_dir, (
        f"DEFAULT_CHECKPOINT_DIR must be {expected_checkpoint_dir} to match manifest, "
        f"got {RFDETRStainBackend.DEFAULT_CHECKPOINT_DIR}"
    )
    assert RFDETRStainBackend.DEFAULT_CONFIG_PATH == expected_config_path, (
        f"DEFAULT_CONFIG_PATH must be {expected_config_path} to match manifest, "
        f"got {RFDETRStainBackend.DEFAULT_CONFIG_PATH}"
    )
    assert RFDETRStainBackend.TRAINING_TOOLKIT_PATH == expected_training_toolkit, (
        f"TRAINING_TOOLKIT_PATH must be {expected_training_toolkit} to match manifest, "
        f"got {RFDETRStainBackend.TRAINING_TOOLKIT_PATH}"
    )
    assert RFDETRStainBackend.RFDETR_SRC_PATH == expected_rfdetr_src, (
        f"RFDETR_SRC_PATH must be {expected_rfdetr_src} to match manifest, "
        f"got {RFDETRStainBackend.RFDETR_SRC_PATH}"
    )
