...
def test_function_gpu_declares_box_only_interactor():
    ...
    assert 'RF-DETR Eagle Combined v5' in manifest


def test_function_gpu_references_both_checkpoint_roots():
    manifest = Path(__file__).with_name('function-gpu.yaml').read_text(encoding='utf-8')

    # Both shape and stain checkpoint directories
    assert 'SHAPE_CHECKPOINT_DIR' in manifest
    assert 'echo-combined-v5/shape_round1' in manifest
    assert 'STAIN_CHECKPOINT_DIR' in manifest
    assert 'echo-combined-v5/stain_round1' in manifest


def test_function_gpu_includes_shape_and_stain_labels():
    manifest = Path(__file__).with_name('function-gpu.yaml').read_text(encoding='utf-8')

    # At least one shape label
    assert '(A13) danno_urto' in manifest
    # At least one stain label
...
def test_function_gpu_mount_paths_consistent_with_opt_bdd():
    ...
    - CHECKPOINT_DIR and CONFIG_PATH correctly use /opt/bdd/runs/echo-combined-v5/shape_round1/...
    ...
    assert 'SHAPE_CHECKPOINT_DIR' in manifest
    assert '/opt/bdd/runs/echo-combined-v5/shape_round1/checkpoints' in manifest
    assert 'SHAPE_CONFIG_PATH' in manifest
    assert '/opt/bdd/runs/echo-combined-v5/shape_round1/config.yaml' in manifest
    
    assert 'STAIN_CHECKPOINT_DIR' in manifest
    assert '/opt/bdd/runs/echo-combined-v5/stain_round1/checkpoints' in manifest
    assert 'STAIN_CONFIG_PATH' in manifest
