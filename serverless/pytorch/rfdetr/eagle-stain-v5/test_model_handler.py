...
class DummyBackend:
    ...
    def predict(self, image, conf_threshold=None):
        ...
        return [
            ...
            PredictedInstance(
                class_name='(C6) superf_bagn_dilav_percolaz',
                score=0.7,
                mask=mask2,
            ),
        ]
...
def test_handle_returns_mapped_shapes_and_skips_unmapped_labels(monkeypatch):
    ...
    shapes = handler.handle(
        ...
        obj_bbox=[[0, 0], [39, 29]],
        mapping={
            '(C5) infiltraz_cls': {'name': 'infiltraz_cls', 'attributes': {}},
        },
    )

    # Should only return the mapped class
    assert len(shapes) == 1
    assert shapes[0]['label'] == 'infiltraz_cls'
...
def test_model_handler_passes_manifest_configured_checkpoint_paths_to_backend(monkeypatch):
    """ModelHandler must pass CHECKPOINT_DIR and CONFIG_PATH env vars into RFDETRStainBackend.
    ...
    monkeypatch.setenv('CHECKPOINT_DIR', '/opt/bdd/runs/echo-combined-v5/stain_round1/checkpoints')
    monkeypatch.setenv('CONFIG_PATH', '/opt/bdd/runs/echo-combined-v5/stain_round1/config.yaml')
    ...
    from pathlib import Path
    assert received_kwargs['checkpoint_dir'] == Path('/opt/bdd/runs/echo-combined-v5/stain_round1/checkpoints')
    assert received_kwargs['config_path'] == Path('/opt/bdd/runs/echo-combined-v5/stain_round1/config.yaml')
