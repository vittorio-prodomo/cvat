...
def test_http_handler_accepts_native_body_forms_and_returns_flat_detector_list(load_argus, body_type, mode):
    main = load_argus("main")
    model = Model()
...
def test_http_handler_passes_native_null_and_threshold_boundaries(load_argus, threshold):
    main = load_argus("main")
    model = Model()
...
def test_http_handler_defaults_missing_threshold_and_leaves_native_roi_coordinates_local(load_argus):
    main = load_argus("main")
    model = Model()
...
def test_http_handler_returns_400_for_malformed_body_without_inference(load_argus, body):
    main = load_argus("main")
    model = Model()
...
def test_http_handler_returns_400_for_invalid_image_without_inference(load_argus, value):
    main = load_argus("main")
    model = Model()
...
def test_http_handler_rejects_bad_threshold_before_inference(load_argus, value):
    main = load_argus("main")
    model = Model()
...
def test_inference_failures_are_logged_and_return_500_never_empty_detections(load_argus, exception):
    main = load_argus("main")
    ...
    assert json.loads(response.body) == {"error": "Argus inference failed"}
    assert any(str(exception) in message for message in context.messages)


def test_init_context_loads_model_eagerly_with_native_logger(load_argus, monkeypatch):
    main = load_argus("main")
    model = Model()
    received = []
