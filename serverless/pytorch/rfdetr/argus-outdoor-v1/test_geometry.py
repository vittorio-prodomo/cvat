...
def test_letterbox_uses_validation_resize_black_padding_and_rounding(
    load_argus, shape, resized, pad
):
    geometry = load_argus("geometry")
...
def test_mask_restoration_removes_odd_padding_and_keeps_received_frame_edges(load_argus, shape):
    geometry = load_argus("geometry")
...
def test_dense_mask_keeps_holes_disconnected_components_and_inclusive_bounds(load_argus):
    geometry = load_argus("geometry")
...
def test_single_edge_pixel_is_valid_dense_mask_and_empty_mask_has_no_shape(load_argus):
    geometry = load_argus("geometry")
...
def test_mask_restoration_preserves_topology_at_native_size(load_argus):
    geometry = load_argus("geometry")
...
def test_restoration_rejects_wrong_model_mask_resolution(load_argus, shape):
    geometry = load_argus("geometry")
...
def test_preprocessing_matches_real_albumentations_validation_transforms(load_argus, monkeypatch):
