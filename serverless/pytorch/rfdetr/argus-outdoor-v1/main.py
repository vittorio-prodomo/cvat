"""Nuclio entry point for the Argus Outdoor v1 standard CVAT detector."""
...
def init_context(context):
    context.logger.info("Initializing Argus Outdoor v1")
    context.user_data.model = ModelHandler(logger=context.logger)
    context.logger.info("Argus Outdoor v1 initialization complete")
...
def handler(context, event):
    ...
    with image:
        context.logger.info(f"Argus request: image_size={image.size} threshold={threshold}")
        try:
            ...
            context.logger.error(f"Argus inference failed: {type(exc).__name__}: {exc}")
            return response(context, json.dumps({"error": "Argus inference failed"}), 500)
    context.logger.info(f"Argus response: detections={len(annotations)}")
