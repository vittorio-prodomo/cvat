...
| Segment Anything 3 (SAM3)                                 | AI Tools | Segment Anything 3 (SAM3) adds a GPU-only prompt-based interactor for image segmentation in the same AI Tools flow. It accepts positive points, negative points, and an optional bounding box, then returns a mask directly from the serverless backend. <br><br>For more information, see: <li>[GitHub: SAM3](https://github.com/facebookresearch/sam3)                                                                                                                                                                                                                                               | ![Example of annotation process using Segment Anything Model](/images/interactors_SAM.gif) |
| Crop YOLO Interactor                                      | Interactor | Draw a crop box on a high-resolution image, run local YOLO instance segmentation on that ROI, and create mapped masks only inside the selected crop.                                                                                                                                                                                                                                                                                                                                                                                                                                                         | - |
